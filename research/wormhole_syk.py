...
Deliberate simplification, stated plainly: real-time evolution here is
exact matrix exponentiation (scipy.linalg.expm via eigendecomposition)
applied directly to the statevector, not a Trotterized gate circuit.
The paper needed gates because that's what real hardware executes (369
two-qubit gates); we have exact statevector access, and the paper's own
hardware results are validated against exactly this kind of "exact
result" baseline. A Trotterized gate-circuit version -- closer to what
real hardware would run, and a candidate for a future Composer/MCP
feature if this line of work continues -- is an explicit, separate
...
def run_wormhole_protocol(n_majorana, k_terms, J, mu, t0, t1, seed, with_message):
    """One run of: TFD (beta=0, i.e. n_majorana/2 Bell pairs L_i-R_i) ->
    P,Q reference/message Bell pair, Q swapped into L[0] if with_message
    -> evolve under H_L+H_R for t0 -> coupling exp(i*mu*V) -> evolve for
    t1 -> mutual information between P and R[0].

    V = (1/(4*n_majorana)) * sum_j chi_L^j chi_R^j (the paper's bilinear
    coupling); mu<0 vs mu>0 is the "traversable" vs "non-traversable"
    sign in the paper's own convention, though note our own random SYK
    realization means our sign doesn't have to line up with theirs --
    what matters is that *some* consistent sign shows the enhancement,
    which mu=-12 does for the seed 61 instance (see __main__).
    """
    n_side = n_majorana // 2
    n_full = 2 * n_side + 2
    ...
    H = de.pauli_hamiltonian_to_matrix(terms_l + terms_r, n_full)
...
if __name__ == '__main__':
    ...
    print(f'\n=== Sign-dependent mutual-information sweep, seed={good_seed} ===')
    mu_mag = 12.0
