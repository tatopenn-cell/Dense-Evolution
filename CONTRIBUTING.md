...
- Any diagnostic quantity computed both eagerly and under `jax.jit` (bond dimension counts, JSD/truncation-error bookkeeping, anything in `_bond_history` and friends) needs a test that compares the two code paths on the *same* input, not just that each path is internally self-consistent — `max_bond_used()` silently over-reported by 2x on some circuits because `_jsd_vectors_jax`'s tail-masking gave the correct (zero) answer eagerly but a wrong one under `jax.jit`+`vmap` for the exact same array; the statevector fidelity was already cross-checked eager-vs-jit and stayed correct throughout, which is exactly why nothing caught it.
