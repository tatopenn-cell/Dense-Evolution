...
- 8 new/updated tests across `tests/test_simulator.py` and `tests/test_autodiff.py`; full suite (702 tests, excluding two server-fixture files run separately due to shared-resource contention when combined with the rest) passes with zero regressions.
