...
**`apply_to_sv`'s randomness source silently depends on the input array's type.** It accepts two different, non-interchangeable randomness arguments — `rng: np.random.Generator` and `jax_key: jax.random.PRNGKey` — and picks which one it actually uses based on `isinstance(sv, jnp.ndarray)`, not on which one you passed:

- `sv` is a NumPy array → uses `rng` (or a fresh default generator if `rng=None`). Reproducible, as expected.
- `sv` is a JAX array (e.g. the statevector returned by `circuit_to_energy_fn`'s `energy_fn`, the common case when composing noise into a `jax.grad`/`jax.value_and_grad` pipeline) → **`rng` is ignored even if you pass it**, and `jax_key` is used instead — if `jax_key` is also `None`, a fresh key is drawn from `os.urandom()` + `time.perf_counter_ns()` on every single call.

Concretely: seeding `rng = np.random.default_rng(42)` and passing it to `apply_to_sv(sv_from_energy_fn, ..., rng=rng)` gives a **different, non-reproducible result every call**, silently — verified directly, two consecutive calls with the identical seeded `rng` and identical input diverged. If you're mixing `NoiseModel` into a differentiable QML/VQE pipeline (statevectors are JAX arrays there), pass `jax_key=jax.random.PRNGKey(...)` explicitly — and split a fresh subkey per call (e.g. `jax.random.split`) if you want a different-but-reproducible noise draw each training step, rather than reusing one key everywhere.
...
- **Docs**: documented `NoiseModel.apply_to_sv`'s split randomness API in the Noise Models section -- `rng` (NumPy) vs `jax_key` (JAX) are picked based on the input statevector's array type, not on which one is actually passed, so seeding `rng` and calling with a JAX statevector (the normal case when composing noise into a `circuit_to_energy_fn`/`jax.grad` pipeline) silently ignores the seed and draws from OS entropy on every call instead -- verified directly (two calls, identical seeded `rng`, identical input, divergent output). Found while reviewing an end-to-end QML+noise+ZNE test script combining `circuit_to_energy_fn`, `NoiseModel`, and the new `zero_noise_extrapolation` under one `jax.value_and_grad` -- gradients did flow correctly through the whole pipeline (that part worked as intended), but the epoch-to-epoch loss instability in that script traced back to this, not to the optimizer or to `mitigation.py`. Not changed here -- see tracked issue for unifying the two call paths.
