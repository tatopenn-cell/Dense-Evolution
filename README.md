...
### v8.1.39
- **Corrected**: v8.1.38's claim that density-matrix ZNE healing (`zne_density_matrix`) is "unreliable for phaseflip and amplitude_damping" was investigated further and found to be wrong -- a Monte Carlo undersampling artifact (`experiments/matrix_healing_zne_sweep.py`'s K=150 trajectories / 3 seeds was too few), not a real limitation of the technique. `richardson_extrapolate`'s 3-point Lagrange coefficients (3, -3, 1) amplify statistical noise in their inputs (sum of squares 19x a single raw measurement's variance) -- an undersampled density-matrix estimate makes the *corrected* result noisy even when the underlying correction is sound. Re-running the exact same previously-failing configurations at higher K (300-1200 trajectories, 10 seeds) turned "unreliable, sometimes net-negative" into consistently and strongly positive.
  - Re-ran the full sweep properly (K=400, 5 seeds, still 2-5 qubits x all 5 `NoiseModel` channels, 100 runs total): **96/100 positive, mean delta +0.12**, every single (qubit count, noise channel) combination net positive on average, growing to +0.20-0.25 at 5 qubits for depolarizing/bitflip. The 4 remaining negative runs are small (worst -0.02), consistent with residual Monte Carlo noise rather than a systematic failure.
  - `zne_density_matrix`'s docstring rewritten with the corrected numbers and an explicit practical caveat for callers: correction quality depends on `rho_at_scales` being a low-noise (large-enough-K) estimate to begin with -- an inherent property of Richardson-style extrapolation's noise amplification, not a bug in this function.

### v8.1.38
- **Documented**: `zne_density_matrix`'s docstring extended with a broader honest finding beyond v8.1.37's original single-configuration result. `experiments/matrix_healing_zne_sweep.py` (new) sweeps GHZ states from 2 to 5 qubits across all 5 `NoiseModel` channels (depolarizing, bitflip, phaseflip, amplitude_damping, combined), 3 seeds each, 60 runs total: 54/60 positive, mean delta +0.09. The effect is consistently positive and *grows with qubit count* for depolarizing/bitflip/combined noise (up to +0.23 at 5 qubits), but is unreliable for phaseflip and amplitude_damping -- small or even net-negative average deltas at some qubit counts (e.g. phaseflip averages -0.01 at 4 qubits). No code behavior changed -- this is a documentation update reflecting a materially fuller picture than the original 2-qubit/depolarizing-only result, so the docstring doesn't understate how conditional the improvement actually is.
